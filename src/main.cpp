...
auto main() -> int {
    ...
    // Since the focus of this book is on OpenGL version 3.3 we'd like to tell GLFW
    // that 3.3 is the OpenGL version we want to use. This way GLFW can make the proper
    // arrangements when creating the OpenGL context. This ensures that when a user
    // does not have the proper OpenGL version GLFW fails to run. We set the major and
    // minor version both to 3. We also tell GLFW we want to explicitly use the
    // core-profile. Telling GLFW we want to use the core-profile means we'll get
    ...
    // argument. The second argument specifies how many strings we're passing as source
    // code, which is only one. The third parameter is the actual source code of the
    // vertex shader and we can leave the 4th parameter to null.
